...
contract WorkSubmission is Ownable, Pausable, ReentrancyGuard {
    ...
    enum SubmissionStatus { Pending, Approved, Rejected, Revised }
    ...
    event SubmissionUpdated(uint256 indexed submissionId, string newWorkIPFSHash, string newCommentIPFSHash);
    event SubmissionStatusChanged(uint256 indexed submissionId, SubmissionStatus newStatus);
    ...
    constructor(address _userManagementAddress, address _jobPostingAddress, address _proposalManagementAddress) Ownable(msg.sender) {
        userManagement = UserManagement(_userManagementAddress);
        jobPosting = JobPosting(_jobPostingAddress);
        proposalManagement = ProposalManagement(_proposalManagementAddress);
    }

    function submitWork(uint256 _jobId, string memory _workIPFSHash, string memory _commentIPFSHash) 
        external 
        whenNotPaused 
        nonReentrant 
    ...
    function updateSubmission(uint256 _jobId, uint256 _submissionId, string memory _newWorkIPFSHash, string memory _newCommentIPFSHash) 
        external 
        whenNotPaused 
        nonReentrant 
        onlyJobFreelancer(_jobId) 
    {
        ...
        uint256 submissionIndex = findSubmissionIndex(submissions, _submissionId);
        require(submissionIndex < submissions.length, "Submission not found");
        require(submissions[submissionIndex].status == SubmissionStatus.Pending || 
        ...
        emit SubmissionUpdated(_submissionId, _newWorkIPFSHash, _newCommentIPFSHash);
        emit SubmissionStatusChanged(_submissionId, SubmissionStatus.Revised);
    }

    function approveSubmission(uint256 _jobId, uint256 _submissionId) 
        external 
        whenNotPaused 
        nonReentrant 
    ...
    function rejectSubmission(uint256 _jobId, uint256 _submissionId, string memory _feedbackIPFSHash) 
        external 
        whenNotPaused 
        nonReentrant 
        onlyJobClient(_jobId) 
    ...
    function getJobSubmissions(uint256 _jobId) 
        external 
        view 
        onlyJobParticipants(_jobId) 
        returns (Submission[] memory) 
    {
        return jobSubmissions[_jobId];
    }

    function findSubmissionIndex(Submission[] storage submissions, uint256 _submissionId) internal view returns (uint256) {
