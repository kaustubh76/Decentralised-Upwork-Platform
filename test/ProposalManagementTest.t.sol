...
contract ProposalManagementTest is Test {
    ...
    function setUp() public {
        ...
        jobPosting = new JobPosting(address(userManagement), address(paymentToken));
        ...
        jobPosting.createJob("ipfs://jobhash", 10 * 10**18, block.timestamp + 7 days);
        vm.stopPrank();
    }

    function testSubmitProposal() public {
        vm.prank(freelancer1);
        proposalManagement.submitProposal(JOB_ID, 9 * 10**18, "ipfs://proposalhash", block.timestamp + 5 days);

        (uint256 id, uint256 jobId, address freelancer, uint256 bidAmount, string memory proposalIPFSHash, uint256 deliveryTime, ProposalManagement.ProposalStatus status, ) = proposalManagement.proposals(1);
    ...
    function testFailSubmitProposalInvalidJob() public {
        vm.prank(freelancer1);
        proposalManagement.submitProposal(999, 9 * 10**18, "ipfs://proposalhash", block.timestamp + 5 days);
    }

    function testUpdateProposal() public {
        vm.startPrank(freelancer1);
        proposalManagement.submitProposal(JOB_ID, 9 * 10**18, "ipfs://proposalhash", block.timestamp + 5 days);
        proposalManagement.updateProposal(1, 8 * 10**18, "ipfs://newhash", block.timestamp + 4 days);
        vm.stopPrank();

        (,, address freelancer, uint256 bidAmount, string memory proposalIPFSHash, uint256 deliveryTime,,) = proposalManagement.proposals(1);
    ...
    function testWithdrawProposal() public {
        ...
        (,,,,,,ProposalManagement.ProposalStatus status,) = proposalManagement.proposals(1);
        assertEq(uint(status), uint(ProposalManagement.ProposalStatus.Withdrawn));
    }
    ...
    //     (,,,,,,ProposalManagement.ProposalStatus status,) = proposalManagement.proposals(1);
    //     assertEq(uint(status), uint(ProposalManagement.ProposalStatus.Accepted));
    ...
    function testGetProposalsByJob() public {
        vm.prank(freelancer1);
        proposalManagement.submitProposal(JOB_ID, 9 * 10**18, "ipfs://proposalhash1", block.timestamp + 5 days);

        vm.prank(freelancer2);
        proposalManagement.submitProposal(JOB_ID, 8 * 10**18, "ipfs://proposalhash2", block.timestamp + 4 days);

        uint256[] memory proposals = proposalManagement.getProposalsByJob(JOB_ID);
    ...
    function testGetProposalsByFreelancer() public {
        ...
        jobPosting.createJob("ipfs://jobhash2", 15 * 10**18, block.timestamp + 14 days);
        vm.stopPrank();

        vm.prank(freelancer1);
        proposalManagement.submitProposal(1, 14 * 10**18, "ipfs://proposalhash2", block.timestamp + 10 days);

        uint256[] memory proposals = proposalManagement.getProposalsByFreelancer(freelancer1);
        assertEq(proposals.length, 2);
        assertEq(proposals[0], 1);
        assertEq(proposals[1], 2);
    }
