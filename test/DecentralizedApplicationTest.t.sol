...
contract UserManagementTest is Test {
    ...
    function testDeployment() public {
        assertTrue(userManagement.hasRole(userManagement.DEFAULT_ADMIN_ROLE(), owner));
    ...
    function testGetUserAddressById() public {
        ...
        address userAddress = userManagement.getUserAddressById(0);
    ...
    */
    ...
    /*
    function testFuzzRegisterUsers(address[] memory users) public {
        for (uint i = 0; i < users.length; i++) {
            if (users[i] != address(0)) {
                vm.prank(users[i]);
                userManagement.registerUser(true);
                UserManagement.User memory user = userManagement.getUser(users[i]);
                assertTrue(user.isActive);
                assertEq(user.userAddress, users[i]);
            }
        }
    }
